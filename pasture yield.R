...
tidy_lm_db <- db %>%
  select(c("DistrictDairyStatistics_2018", "year_ending", "Pasture.and.Crop.harvested.t.DM.ha")) %>%  #select only essential columns. Data to nest within region, because multiple observations for each region at each year
  nest(yields = c(year_ending, Pasture.and.Crop.harvested.t.DM.ha)) %>%
  mutate(model = map(yields, ~ lm(Pasture.and.Crop.harvested.t.DM.ha ~ year_ending, data = .x)))
...
library(ggrepel)
slopes_db %>%
  ggplot(aes(estimate, p.value, label = DistrictDairyStatistics_2018, colour = DistrictDairyStatistics_2018 )) +
  geom_vline(
    xintercept = 0, lty = 2,
    size = 1.5, alpha = 0.7, color = "gray50"
  ) +
